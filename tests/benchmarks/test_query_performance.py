...
from .benchmark_config import BenchmarkConfig, BenchmarkResult, BenchmarkThresholds
...
class TestQueryPerformance:
    ...
    async def test_concurrent_query_throughput(self, benchmark_session):
        ...
        # Verify thresholds
        assert throughput >= thresholds.min_throughput_async, \
            f"Throughput {throughput:.1f} qps below threshold {thresholds.min_throughput_async} qps"
        assert successful >= num_queries * 0.99, \
    ...
    async def test_async_vs_sync_performance(self, benchmark_session):
        ...
        # Verify thresholds
        assert speedup >= thresholds.concurrency_speedup_factor, \
            f"Async speedup {speedup:.1f}x below threshold {thresholds.concurrency_speedup_factor}x"
        assert async_throughput >= thresholds.min_throughput_async, \
    ...
    async def test_query_latency_under_load(self, benchmark_session):
        ...
        assert avg_latency < thresholds.single_query_avg * 2, \
            f"Average latency under load {avg_latency:.3f}s exceeds 2x threshold"
        assert p99_latency < thresholds.single_query_p99 * 2, \
    ...
    async def test_prepared_statement_performance(self, benchmark_session):
        ...
        assert prepared_duration < simple_duration, \
            "Prepared statements should be faster overall"
        assert prepared_avg < simple_avg, \
            "Prepared statements should have lower average latency"
        assert performance_gain > 0.1, \

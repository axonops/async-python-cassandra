...
from .benchmark_config import BenchmarkConfig, BenchmarkThresholds
...
class TestStreamingPerformance:
        ...
        for partition in range(100):
            ...
            for i in range(0, len(batch), batch_size):
                await asyncio.gather(*[
                    ...
                    for params in batch[i:i+batch_size]
                ])
    ...
    async def test_streaming_memory_efficiency(self, benchmark_session):
        ...
        regular_result = await benchmark_session.execute(
            "SELECT * FROM streaming_test LIMIT 10000"
        )
        regular_rows = []
        ...
        # Verify thresholds
        assert memory_ratio < thresholds.streaming_memory_overhead, \
            f"Streaming memory ratio {memory_ratio:.2f} exceeds threshold {thresholds.streaming_memory_overhead}"
        assert stream_memory_used < regular_memory_used, \
    ...
    async def test_streaming_throughput(self, benchmark_session):
        ...
        stream_result = await benchmark_session.execute_stream(
            "SELECT * FROM streaming_test LIMIT 50000",
            stream_config=stream_config
        )
        ...
        assert row_count == 50000, \
            f"Expected 50000 rows, got {row_count}"
    ...
    async def test_streaming_latency_overhead(self, benchmark_session):
        ...
        for size in test_sizes:
            ...
            regular_result = await benchmark_session.execute(f"SELECT * FROM streaming_test LIMIT {size}")
            regular_rows = []
            async for row in regular_result:
                regular_rows.append(row)
            ...
            # Calculate overhead
            overhead_ratio = stream_duration / regular_duration if regular_duration > 0 else float('inf')
            
            # Verify overhead is acceptable
            assert overhead_ratio < thresholds.streaming_latency_overhead, \
                f"Streaming overhead {overhead_ratio:.2f}x for {size} rows exceeds threshold"
            assert len(stream_rows) == len(regular_rows), \
    ...
    async def test_streaming_page_processing_performance(self, benchmark_session):
        ...
        # Verify performance
        assert avg_page_latency < 0.1, \
            f"Average page processing time {avg_page_latency:.3f}s exceeds 100ms"
        assert page_throughput > 10, \
            f"Page throughput {page_throughput:.1f} pages/sec below minimum"
        assert row_throughput > 5000, \
            f"Row throughput {row_throughput:.0f} rows/sec below minimum"
    ...
    async def test_concurrent_streaming_operations(self, benchmark_session):
        ...
        results = await asyncio.gather(*[
            stream_worker(i) for i in range(num_workers)
        ])
        ...
        assert total_duration < avg_worker_duration * 2, \
            "Concurrent streams should show parallelism benefit"
        assert all(count >= 900 for _, count in results), \
            "All workers should process most of their rows"
        assert total_rows >= num_workers * 900, \
            f"Total rows {total_rows} below expected minimum"
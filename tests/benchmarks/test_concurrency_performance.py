...
class TestConcurrencyPerformance:
    ...
    async def test_high_concurrency_throughput(self, benchmark_session):
        ...
        assert successful >= num_operations * 0.99, \
    ...
    async def test_connection_pool_efficiency(self, benchmark_session):
        ...
        try:
            select_stmt = await limited_session.prepare(
                "SELECT * FROM concurrency_test LIMIT 1"
            )
            ...
            query_times = await asyncio.gather(*[
                timed_query(i) for i in range(num_queries)
            ])
            ...
            assert avg_time < 0.1, \
                f"Average query time {avg_time:.3f}s indicates pool contention"
            assert p95_time < 0.2, \
                f"P95 query time {p95_time:.3f}s indicates severe contention"
    ...
    async def test_resource_usage_under_load(self, benchmark_session):
        ...
        # Prepare statement
        select_stmt = await benchmark_session.prepare(
            "SELECT * FROM concurrency_test LIMIT 10"
        )
        ...
        assert avg_memory_increase < 100, \
        ...
        assert avg_cpu < 80, \
            f"Average CPU usage {avg_cpu:.1f}% exceeds 80%"
    ...
    async def test_concurrent_operation_isolation(self, benchmark_session):
        ...
        results = await asyncio.gather(*[
            increment_counter() for _ in range(num_increments)
        ])
        ...
        assert throughput > 100, \
            f"Increment throughput {throughput:.1f} ops/sec too low"
    ...
    async def test_graceful_degradation_under_overload(self, benchmark_session):
        ...
        start_time = time.perf_counter()
        results = await asyncio.gather(*[
            overload_operation(i) for i in range(num_operations)
        ...
        assert error_rate < 0.5, \
            f"Error rate {error_rate:.1%} too high"
        
        # Latencies will be high but should be bounded
        assert p99_latency < 5.0, \
            f"P99 latency {p99_latency:.1f}s exceeds 5 second timeout"
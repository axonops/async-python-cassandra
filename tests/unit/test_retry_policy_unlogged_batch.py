...
from cassandra.policies import RetryPolicy, ConsistencyLevel
...
class TestUnloggedBatchRetry:
    ...
    def test_different_batch_types_behavior(self):
        ...
        for write_type, has_attr, attr_value, should_retry in test_cases:
            ...
            if should_retry:
                assert decision == RetryPolicy.RETRY, f"Failed for {write_type} with idempotent={attr_value}"
            else:
                assert decision == RetryPolicy.RETHROW, f"Failed for {write_type} with idempotent={attr_value}"

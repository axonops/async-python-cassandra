...
from async_cassandra.metrics import (
    QueryMetrics, ConnectionMetrics, MetricsMiddleware,
    InMemoryMetricsCollector, PrometheusMetricsCollector,
...
class TestMetricsMiddleware:
    ...
    async def test_middleware_connection_check(self):
        ...
        mock_session.cluster.metadata.all_hosts = Mock(return_value=[
            Mock(address="127.0.0.1", is_up=True)
        ])
...
class TestCreateMetricsSystem:
    ...
    def test_create_with_custom_collector(self):
        ...
        metrics = create_metrics_system(
            backend="memory",
            additional_collectors=[custom_collector]
        )

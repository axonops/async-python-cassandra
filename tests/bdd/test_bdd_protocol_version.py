...
@scenario("features/protocol_version_enforcement.feature", "Connection to Cassandra 3.11 should fail with clear error")
def test_cassandra_311_connection_fails():
    """Test connection to Cassandra 3.11 fails."""
    pass


@scenario("features/protocol_version_enforcement.feature", "Explicit protocol v5 with Cassandra 3.11 should fail immediately")
def test_cassandra_311_explicit_v5_fails():
    """Test explicit v5 with Cassandra 3.11 fails."""
    pass


@scenario("features/protocol_version_enforcement.feature", "Explicit protocol v4 should be rejected at configuration time")
def test_explicit_v4_rejected():
    """Test explicit v4 is rejected."""
    pass


@scenario("features/protocol_version_enforcement.feature", "Connection to Cassandra 5 should succeed with protocol v5 or higher")
def test_cassandra_5_connection_succeeds():
    """Test connection to Cassandra 5 succeeds."""
    pass


@scenario("features/protocol_version_enforcement.feature", "Explicit protocol v5 with Cassandra 5 should succeed")
def test_cassandra_5_explicit_v5_succeeds():
    """Test explicit v5 with Cassandra 5 succeeds."""
    pass


@scenario("features/protocol_version_enforcement.feature", "Explicit protocol v6 with Cassandra 5 should fail due to beta status")
...
def cassandra_version_running(version, port, protocol_context):
    ...
    protocol_context["container_info"] = {"version": version, "port": port, "container": container_name}
...
def try_create_cluster_with_version(version, protocol_context):
    ...
    port = protocol_context["container_info"]["port"] if protocol_context.get("container_info") else 9042
...
def verify_no_host_error(protocol_context):
    ...
    acceptable_errors = ["NoHostAvailable", "protocol", "Protocol", "Beta version", "Cluster is already shut down"]
    assert any(err in error_msg for err in acceptable_errors), f"Expected protocol/NoHostAvailable error, got: {error_msg}"
...
def verify_protocol_incompatibility(protocol_context):
    ...
    protocol_keywords = ["protocol", "version", "v5", "Cassandra 4.0", "Beta version", "Cluster is already shut down"]
    assert any(keyword in error_msg for keyword in protocol_keywords), f"Expected protocol error, got: {error_msg}"
...
def verify_success(protocol_context):
    ...
    result = protocol_context["connection_result"]
    assert result["success"], f"Connection should have succeeded but failed with: {result.get('error')}"

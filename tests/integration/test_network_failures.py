...
async def session_with_retry(cassandra_container) -> AsyncCassandraSession:
    ...
    retry_policy = AsyncRetryPolicy(
        read_retry_count=2, write_retry_count=2, unavailable_retry_count=2
    )
...
class TestNetworkFailures:
    ...
    async def test_high_latency_operations(
        self, session_with_retry: AsyncCassandraSession, network_manipulator
    ):
        ...
        select_stmt = await session_with_retry.prepare("SELECT * FROM failure_test WHERE id = ?")
        ...
        await session_with_retry.execute(
            insert_stmt, [test_id, "high_latency_test", 1, datetime.utcnow()]
        )
    ...
    async def test_packet_loss_resilience(
        ...
        for i in range(50):
            ...
            try:
                start = time.time()
                await session_with_retry.execute(
                    insert_stmt, [test_id, f"packet_loss_test_{i}", i, datetime.utcnow()]
                )
    ...
    async def test_request_timeout_handling(self, session_with_retry: AsyncCassandraSession):
        ...
        for i in range(1000):
            await session_with_retry.execute(
                insert_stmt, [uuid.uuid4(), "x" * 1000, i, datetime.utcnow()]
            )

        # Try to query all with very short timeout
        with pytest.raises(OperationTimedOut):
            await session_with_retry.execute(
                "SELECT * FROM failure_test", timeout=0.001  # 1ms timeout
            )
    ...
    async def test_node_failure_recovery(
        ...
        test_id = uuid.uuid4()
        await session_with_retry.execute(
            insert_stmt, [test_id, "recovery_test", 1, datetime.utcnow()]
        )
    ...
    async def test_concurrent_failures(self, session_with_retry: AsyncCassandraSession):
        ...
        async def failing_operation(should_fail: bool):
            ...
            if should_fail:
                ...
                try:
                    await session_with_retry.execute(
                        "INSERT INTO nonexistent_table (id) VALUES (?)", [test_id]
                    )
                ...
                await session_with_retry.execute(
                    insert_stmt, [test_id, "concurrent_test", 1, datetime.utcnow()]
                )
                return "success"
    ...
    async def test_retry_policy_effectiveness(self, cassandra_container):
        ...
        cluster_with_retry = AsyncCluster(
            ...
            request_timeout=1.0,
            retry_policy=AsyncRetryPolicy(
                read_retry_count=3, write_retry_count=3, unavailable_retry_count=3
            ),
        )
    ...
    async def test_streaming_with_failures(self, session_with_retry: AsyncCassandraSession):
        ...
        for i in range(1000):
            await session_with_retry.execute(
                insert_stmt, [uuid.uuid4(), f"stream_test_{i}", i, datetime.utcnow()]
            )
        ...
        try:
            result = await session_with_retry.execute_stream(
                "SELECT * FROM failure_test", stream_config=config
            )
    ...
    async def test_batch_operations_with_failures(self, session_with_retry: AsyncCassandraSession):
        ...
        for size in batch_sizes:
            ...
            for i in range(size):
                batch.add(insert_stmt, [uuid.uuid4(), f"batch_{size}_{i}", i, datetime.utcnow()])

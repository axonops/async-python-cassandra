...
class TestLongLivedConnections:
    ...
    async def test_session_survives_errors(self, cassandra_cluster):
        ...
        test_id = uuid.uuid4()
        await session.execute(
            "INSERT INTO test_errors (id, data) VALUES (%s, %s)",
            [test_id, "test data"]
        )
        ...
        result = await session.execute(
            "SELECT * FROM test_errors WHERE id = %s",
            [test_id]
        )
    ...
    async def test_prepared_statements_are_cached(self, cassandra_cluster):
        ...
        prepared2 = await session.prepare(
            "SELECT cluster_name FROM system.local WHERE key = ?"
        )

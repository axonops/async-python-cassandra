...
class TestStressScenarios:
    ...
    async def test_extreme_concurrent_writes(self, stress_session: AsyncCassandraSession):
        ...
        async def write_record(i: int):
            ...
            try:
                await stress_session.execute(insert_stmt, [
                    uuid.uuid4(),
                    datetime.utcnow(),
    ...
    async def test_sustained_load(self, stress_session: AsyncCassandraSession):
        ...
        metrics_by_second = defaultdict(lambda: {
        ...
        for second in sorted(metrics_by_second.keys()):
            ...
            avg_write_ms = statistics.mean(metrics['write_latencies']) * 1000 if metrics['write_latencies'] else 0
            avg_read_ms = statistics.mean(metrics['read_latencies']) * 1000 if metrics['read_latencies'] else 0
            
            print(f"{second:6d} | {metrics['writes']:8d} | {metrics['reads']:7d} | "
    ...
    async def test_wide_row_performance(self, stress_session: AsyncCassandraSession):
        ...
        print(f"Average time to read {columns_per_partition} columns: {statistics.mean(read_times)*1000:.2f}ms")
        ...
        print(f"Streamed {total_streamed} rows in {stream_time:.2f}s "
    ...
    async def test_connection_pool_limits(self, stress_session: AsyncCassandraSession):
        ...
        for i in range(100):
            await stress_session.execute(insert_stmt, [
                uuid.uuid4(), datetime.utcnow(), f"test_data_{i}",
                {"metric": float(i)}, {f"tag{i}"}
            ])

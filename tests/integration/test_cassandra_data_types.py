...
class TestCassandraDataTypes:
    ...
    async def test_numeric_types(self, setup_cluster):
        ...
        assert row.decimal_val == decimal.Decimal("123456789.123456789")
    ...
    async def test_text_types(self, setup_cluster):
        ...
        # Verify
        for case in test_cases:
            result = await session.execute("SELECT * FROM text_types WHERE id = ?", [case[0]])
            row = result.one()
    ...
    async def test_binary_types(self, setup_cluster):
        ...
        for id_val, expected_blob in test_data:
            result = await session.execute("SELECT * FROM binary_types WHERE id = ?", [id_val])
            row = result.one()
    ...
    async def test_frozen_collections(self, setup_cluster):
        ...
        assert row.frozen_list == tuple(list_data)  # Frozen lists become tuples
    ...
    async def test_tuple_type(self, setup_cluster):
        ...
        assert row.tuple_val == tuple_data
    ...
    async def test_prepared_statements_with_types(self, setup_cluster):
        ...
        # Query and verify
        result = await session.execute("SELECT * FROM prepared_types WHERE id = ?", [test_id])
        row = result.one()

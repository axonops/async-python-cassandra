...
import asyncio
from typing import AsyncIterator, Optional, Callable, List, Union, Any
...
class AsyncStreamingResultSet:
    ...
    def _handle_page(self, rows: List[Any]) -> None:
        ...
        if rows is not None:
            ...
            if (self.config.max_pages and 
                self._page_number >= self.config.max_pages):
...
class StreamingResultHandler:
    ...
    def __init__(self, response_future: ResponseFuture, config: Optional[StreamConfig] = None):
        ...
        self.response_future.add_callbacks(
            callback=self._handle_initial_response,
            errback=self._handle_initial_error
        )
    ...
    def _handle_initial_response(self, rows: List[Any]) -> None:
        ...
        self._streaming_result = AsyncStreamingResultSet(
        ...
        loop.call_soon_threadsafe(
            self._initial_future.set_result, 

...
class TestFastAPIExample:
    ...
    async def test_thread_safety_under_concurrency(self, app_client):
        ...
        results = await asyncio.gather(*[
            create_and_read_user(i) for i in range(num_concurrent)
    ...
    async def test_streaming_memory_efficiency(self, app_client):
        ...
        stream_resp = await app_client.get(
            f"/users/stream?limit={batch_size}&fetch_size=10"
        )
        ...
        print(f"✓ Streamed {len(stream_data['users'])} users in {stream_data['metadata']['pages_fetched']} pages")
        ...
        print(f"✓ Page-by-page streaming: {pages_data['total_rows_processed']} rows in {len(pages_data['pages_info'])} pages")
    ...
    async def test_error_handling_consistency(self, app_client):
        ...
        invalid_email_resp = await app_client.post("/users", json={
            "name": "Test",
            "email": "not-an-email",
            "age": 25
        })
    ...
    async def test_performance_comparison(self, app_client):
        ...
        print(f"✓ Speedup factor: {async_data['requests_per_second'] / sync_data['requests_per_second']:.1f}x")
    ...
    async def test_timeout_handling(self, app_client):
        ...
        timeout_resp = await app_client.get(
            "/slow_query",
        ...
        success_resp = await app_client.get(
            "/slow_query",

...
from async_cassandra.monitoring import ConnectionMonitor, RateLimitedSession, create_monitored_session
...
async def test_timeout_handling(
    operation: str = Query("connect", regex="^(connect|prepare|execute)$"),
...
async def test_concurrent_load(
    ...
    query_type: str = Query("read", regex="^(read|write)$"),
):
...
async def cleanup_test_users():
    ...
    try:
        ...
        result = await session.execute("SELECT id FROM users WHERE name LIKE 'LoadTest%' ALLOW FILTERING")
        
        deleted_count = 0
        async for row in result:
            await session.execute("DELETE FROM users WHERE id = ?", [row.id])
            deleted_count += 1

...
def export_table_sync(session, table_name: str, output_file: str):
    ...
    async def _export():
        ...
        config = StreamConfig(
            fetch_size=5000,
            page_callback=lambda p, t: logger.info(
                f"Exported {t:,}/{total_rows:,} rows ({100*t/total_rows:.1f}%)"
        ...
        for j in range(batch_size):
            ...
            tasks.append(
                session.execute(insert_stmt, [
                    ...
                    {"color": f"color{product_id % 10}", 
                     "size": f"size{product_id % 4}"},
                    datetime.now()
                ])
            )
    ...
    try:
        ...
        if ASYNC_FILE_IO:
            await export_table_async(
                session,
                "products",
                str(output_dir / "products_async.csv")
            )
        else:
            await export_table_sync(
                session,
                "products",
                str(output_dir / "products_sync.csv")
            )
